...
export class CreationHelper {
    ...
    createTestFile() {
        ...
        if (fs.existsSync(testFilePath)) {
            ...
            throw new Error(`Test file already exists at ${this.getRelativePath(testFilePath)}.`);
        }

        // Write the default template or template specified by the user in the test file.
        this.writeContentToFile(testFilePath).then((success: boolean) => {
            if (success) {
                if (this.configuration.shouldSwitchToTestFile()) {
                    this.switchToTestFile(testFilePath);
                }
    ...
    private shouldCreateTestDirectory(dirPath: string): boolean {
        return dirPath.indexOf(path.sep + this.configuration.getTestDirectoryName() + path.sep) === -1;
    }
